...
public class lessons1829 {
    ...
    public int[] solution(int m, int n, int[][] picture) {
        ...
        boolean[][] visited = new boolean[m][n]; // 방문 여부 배열
            ...
            for (int j = 0; j < picture[0].length; j++) {
                ...
                while (!queue.isEmpty()) {
                    ...
                    for (int k = 0; k < 4; k++) { // 상하좌우 비교하기
                        int nextRow = row + moveRow[k]; // 상하 이동후 좌표
                        int nextCol = col + moveCol[k]; // 좌우 이동후 좌표
                        ...
                        if (picture[row][col] == picture[nextRow][nextCol]) { // 같은 색이라면
                        ...
                        queue.add(new Node(nextRow, nextCol)); // 후보노드에 추가
                    }
